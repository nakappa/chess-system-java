...
public class ChessMatch {
	...
	public ChessPiece[][] getPieces(){
		...
		for (int i = 0; i < board.getRows(); i++) {
			for (int j = 0; j < board.getColumns(); j++) mat[i][j] = (ChessPiece) board.piece(i, j);
		}
		return mat;
	}
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == null) {
				...
				if (p.getColor() == Color.WHITE) pawmPosition = new Position(target.getRow() + 1, target.getColumn());
				
				else  pawmPosition = new Position(target.getRow() - 1, target.getColumn());
				
				capturedPiece = board.removePiece(pawmPosition);
				capturedPieces.add(capturedPiece);
				piecesOnTheBoard.remove(capturedPiece);
			}
		}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				...
				Position pawmPosition;
				if (p.getColor() == Color.WHITE) pawmPosition = new Position(3, target.getColumn());
				
				else  pawmPosition = new Position(4, target.getColumn());
